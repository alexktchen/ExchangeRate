    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0{
            if let item = UserDefaultDataService.sharedInstance.getMajorCurrencysData() {
                cell.textLabel?.text = "today_view_detail_major_currency".localized
                cell.detailTextLabel?.text = item.displayName
            } else {
                cell.textLabel?.text = "today_view_detail_no_selected".localized
                cell.detailTextLabel?.text = ""
            }
        } else {
            if let item = UserDefaultDataService.sharedInstance.getMinorCurrencysData() {
                cell.textLabel?.text = "today_view_detail_minor_currency".localized
                cell.detailTextLabel?.text = item.displayName
            } else {
                cell.textLabel?.text = "today_view_detail_no_selected".localized
                cell.detailTextLabel?.text = ""
